...
Возвращает локали, известные из зарегистрированных loaders.
...
Возвращает все локали, на которые можно переключиться: локали лоадеров **плюс**
те, чьи сообщения уже смержены. У приложения, задающего словари через
`mergeMessages()`, лоадеров нет вовсе, и `getKnownLocales()` вернул бы для него
`[]` — переключателю языка нужен именно этот метод.

```typescript
declare function getAvailableLocales(): Locale[];
```

Чтение реактивно: вызов внутри `computed` или шаблона пересчитается, когда
...
Это единственный честный способ задать вопрос. Сравнение `t(key)` с ключом врёт
на сообщении, значение которого совпадает с его собственным ключом, — реальный
случай в технических словарях кодов и идентификаторов.

Без явной `locale` действует то же разрешение, что и у `t()`: текущая локаль,
затем `fallbackLocale`. С явной — проверяется только она.

Правила приёма значения совпадают с `t()`: строки, числа, булевы, функции-сообщения
и наборы плюральных форм считаются существующими; пространство имён — нет.
...
Для листа возвращается `undefined` — лист читается через `t()`. Набор плюральных
форм тоже лист, хотя и объект.

Поддерево отдаётся `readonly`: мутация обошла бы `mergeMessages()` и оставила
кэш компиляции устаревшим. Локаль резолвится по тому же правилу, что и у `te()`.
