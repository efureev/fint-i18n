...
### Migration: глобальная аугментация типов теперь opt-in

Рантайм-регистрацию `$t`, `$i18n` и директивы `v-t` выполняет `installI18n()`
(при включённом `globalInstall`). Однако **TypeScript-типизация в шаблонах** для
них больше не подключается автоматически — её нужно включить явно, один раз
импортировав модуль аугментации (например, в entry-файле приложения или в любом
`*.d.ts`, который видит ваш `tsconfig`):

```typescript
import '@feugene/fint-i18n/vue/global-types'
```

Это объявляет `ComponentCustomProperties.$t` / `$i18n` и типы директивы `v-t`,
чтобы шаблоны проходили проверку типов под Volar / `vue-tsc`.

> [!IMPORTANT]
> Раньше эта аугментация подключалась за вас как побочный эффект импорта
> библиотеки. Если вы полагались на типизацию `$t`/`$i18n`/`v-t` в шаблонах,
> добавьте импорт выше после обновления — поведение в рантайме не изменилось.
> Это оставляет глобальную область чистой для потребителей, использующих только
> композаблы или директиву.
