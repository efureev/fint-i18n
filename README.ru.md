...
- **Плюрализация по CLDR**: формы выбираются через `Intl.PluralRules`, правила резолвятся при компиляции.
...
- 🖥️ **[Серверный рендеринг](./docs/ru/ssr.md)**: Перенос загруженного на сервере состояния на клиент.
- 🧰 **[Инструменты для словарей](./docs/ru/tooling.md)**: Сверка локалей между собой и с кодом, миграция плюрализации.
